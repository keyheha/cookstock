        ...
        def wrapper(*args, **kwargs):
            ...
            if show_args:
                try:
                    ...
                    if level == "debug":
                        logger.info(
                            "Entering %s args=%s kwargs=%s", name, arg_preview, kwargs
                        )
                    else:
                        logger.info("Entering %s", name)
                except Exception:
                    logger.info(
                        "Entering %s (args omitted due to formatting error)", name
                    )
            else:
                if level == "debug":
...
def find_path():
    ...
    logger.warning(
        "Could not find 'cookstock' project root; returning None. Set COOKSTOCK_PATH env var to override."
    )
    return None
...
if not basePath:
    logger.error(
        "'cookstock' root not found; please set COOKSTOCK_PATH env var or run from inside the repo."
    )
    raise RuntimeError(
        "'cookstock' root not found; set COOKSTOCK_PATH or run from inside repo"
    )
...
class cookFinancials:
    ...
    def __init__(self, ticker, priceData=None, fetch_days=None):
        ...
        if priceData:
            ...
            if cached is not None:
                logger.info(
                    "Loaded cached historical price data for %s (last %d days)",
                    self.ticker,
                    days,
                )
                ...
                logger.info(
                    "Fetching last %d days historical price data for %s",
                    days,
                    self.ticker,
                )
                try:
                    ...
                    logger.info(
                        "Fetching data from %s to %s", str(start_date), str(date)
                    )
                    ...
                    logger.exception(
                        "Failed to fetch historical price data for %s", self.ticker
                    )
        ...
        try:
            ...
            if not self.current_stickerPrice:
                ...
                if self.priceData.get(self.ticker, {}).get("prices"):
                    self.current_stickerPrice = self.priceData[self.ticker]["prices"][
                        -1
                    ]["close"]
    ...
    def get_balanceSheetHistory_quarter(self):
        ...
        self.bshData_quarter = {
            self.ticker: self._convert_financials_to_old_format(bs_q)
        }
    ...
    def get_incomeStatementHistory_quarter(self):
        ...
        self.ish_quarter = {
            self.ticker: self._convert_financials_to_old_format(income_q)
        }
    ...
    def get_BV_quarter(self, numofQuarter=20):
        ...
        for i in range(min(np.size(self.bshData_quarter[self.ticker]), numofQuarter)):
            ...
            if not (self.bshData_quarter[self.ticker][i][date_key]):
                break
            if not (
                self.bshData_quarter[self.ticker][i][date_key].get("stockholdersEquity")
            ):
                # warning
                logger.warning(
                    "stockholdersEquity is not in the dictionary for %s (quarter)",
                    self.ticker,
                )
                break
            bv.append(
                self.bshData_quarter[self.ticker][i][date_key]["stockholdersEquity"]
            )
        return bv
    ...
    def get_ROIC(self, numofYears=20):
        ...
        for i in range(min(np.size(self.bshData[self.ticker]), numofYears)):
            ...
            if self.bshData[self.ticker][i][date_key].get(
                "shortLongTermDebt"
            ) is None or not (
                self.bshData[self.ticker][i][date_key]["shortLongTermDebt"]
            ):
                debt_short = 0
            else:
                debt_short = self.bshData[self.ticker][i][date_key].get(
                    "shortLongTermDebt"
                )
            if self.bshData[self.ticker][i][date_key].get(
                "longTermDebt"
            ) is None or not (self.bshData[self.ticker][i][date_key]["longTermDebt"]):
                debt_long = 0
            else:
    ...
    def get_totalCashFromOperatingActivities(self, numofYears=20):
        ...
        for i in range(min(np.size(self.cfsh[self.ticker]), numofYears)):
            ...
            if not (self.cfsh[self.ticker][i][date_key].get("operatingCashFlow")):
                # warning
                logger.warning(
                    "operatingCashFlow is not in the dictionary for %s", self.ticker
                )
                break
    ...
    def get_ma_ref(self, date_from, date_to):
        ...
        for i in range(len(data[self.ticker]["prices"])):
            ...
            if not (data[self.ticker]["prices"][i]["close"]):
                if i > 0:
                    data[self.ticker]["prices"][i]["close"] = data[self.ticker][
                        "prices"
                    ][i - 1]["close"]
                else:
    ...
    def get_ma(self, date_from, date_to):
        ...
        selectedPriceDataStruct = self.get_price_from_buffer_start_end(
            priceDataStruct, date_from, date_to
        )
        ...
        for i in range(len(selectedPriceDataStruct)):
            ...
            if not (selectedPriceDataStruct[i]["close"]):
                selectedPriceDataStruct[i]["close"] = selectedPriceDataStruct[i - 1][
                    "close"
                ]
    ...
    def get_ema(self, date, period):
        ...
        if not selectedPriceDataStruct or len(selectedPriceDataStruct) < period:
            logger.warning(
                "get_ema: Insufficient data for %s (period=%d)", self.ticker, period
            )
            return -1
    ...
    def is_swing_trade_entry(self):
        ...
        try:
            ...
            if current_price is None:
                logger.warning(
                    "is_swing_trade_entry: No current price for %s", self.ticker
                )
                return False, {}
            ...
            if ema_8 == -1:
                logger.warning(
                    "is_swing_trade_entry: Could not calculate 8 EMA for %s",
                    self.ticker,
                )
                return False, {}
            ...
            if sma_200 == -1:
                logger.warning(
                    "is_swing_trade_entry: Could not calculate 200 SMA for %s",
                    self.ticker,
                )
                return False, {}
    ...
    def check_ma_alignment(self):
        ...
        try:
            ...
            logger.info(
                "MA alignment for %s: 50=%.2f, 150=%.2f, 200=%.2f, required=%s, preferred=%s",
                self.ticker,
                ma_50 if ma_50 != -1 else 0,
                ma_150,
                ma_200,
                meets_required,
                meets_preferred,
    ...
    def get_30day_trend(self):
        ...
        # get 30 days data
        price30Structure = self.get_price_from_buffer(
            self.priceData[self.ticker]["prices"],
    ...
    def mv_strategy(self):
        ...
        if not (currentPrice > price150 and currentPrice > price200):
            logger.info(
                "mv_strategy %s: Price not above both 150 & 200 SMA", self.ticker
            )
            return -1
        ...
        if price50 != -1 and currentPrice > price50:
            logger.info(
                "mv_strategy %s: All Minervini criteria met (including 50 SMA)",
                self.ticker,
            )
        else:
    ...
    def get_vol(self, checkDays, avrgDays):
        ...
        if length < checkDays:
            logger.warning(
                "get_vol: Insufficient data for %s (length=%d, checkDays=%d)",
                self.ticker,
                length,
                checkDays,
            )
        ...
        for i in range(checkDays):
            ...
            if idx < 0 or idx >= length:
                logger.warning(
                    "get_vol: Index out of bounds for %s (idx=%d, length=%d)",
                    self.ticker,
                    idx,
                    length,
                )
            ...
            if not (self.priceData[self.ticker]["prices"][idx]["volume"]):
                ...
                if idx + 1 < length:
                    self.priceData[self.ticker]["prices"][idx]["volume"] = (
                        self.priceData[self.ticker]["prices"][idx + 1]["volume"]
                    )
                else:
        ...
        if length < checkDays + 1:
            logger.warning(
                "get_vol: Insufficient data for avgDays calculation for %s (length=%d, checkDays=%d)",
                self.ticker,
                length,
                checkDays,
            )
        ...
        for i in range(np.min([avrgDays, length - checkDays])):
            ...
            if idx < 0 or idx >= length:
                logger.warning(
                    "get_vol: Index out of bounds for avgDays for %s (idx=%d, length=%d)",
                    self.ticker,
                    idx,
                    length,
                )
    ...
    def vol_strategy(self):
        ...
        if (
            avgVol3day >= algoParas.PEAK_VOL_RATIO * avgVol50day
            and avgVol50day >= algoParas.VOLUME_THRESHOLD
        ):
    ...
    def price_strategy(self):
        ...
        if length == 0:
            logger.warning(
                "price_strategy: No price data available for %s", self.ticker
            )
            return -1
        ...
        for i in range(length):
            if not (self.priceData[self.ticker]["prices"][i]["close"]):
                if i > 0:
                    self.priceData[self.ticker]["prices"][i]["close"] = self.priceData[
                        self.ticker
                    ]["prices"][i - 1]["close"]
                else:
        ...
        if currentPrice is None:
            logger.warning(
                "price_strategy: No current price available for %s", self.ticker
            )
            return -1
        ...
        if highestPrice == lowestPrice:
            logger.warning(
                "price_strategy: No price variation for %s (highestPrice=%s, lowestPrice=%s)",
                self.ticker,
                highestPrice,
                lowestPrice,
            )
        ...
        if range_position >= algoParas.PRICE_POSITION_LOW:
            logger.info(
                "price_strategy %s: Within top 25%% of 52-week high (%.1f%%)",
                self.ticker,
                range_position * 100,
            )
        ...
        logger.info(
            "price_strategy %s: Not in top 25%% (at %.1f%%)",
            self.ticker,
            range_position * 100,
        )
    ...
    def get_price(self, startDate, frame):
        ...
        # don't need to pull data from remote, use local
        priceDataStruct = self.priceData[self.ticker]["prices"]
        selectedPriceDataStruct = self.get_price_from_buffer(
            priceDataStruct, startDate, frame
        )
    ...
    def get_footPrint(self):
        ...
        self.m_footPrint = []
        for i in range(length):
            self.m_footPrint.append(
                [
                    self.m_recordVCP[i][0],
                    self.m_recordVCP[i][2],
                    (self.m_recordVCP[i][1] - self.m_recordVCP[i][3])
                    / self.m_recordVCP[i][1],
                ]
            )
    ...
    def is_pivot_good(self):
        ...
        if not self.m_footPrint or not self.m_recordVCP:
            logger.warning(
                "is_pivot_good: No footprint or VCP records for %s", self.ticker
            )
        ...
        if current is None:
            logger.warning(
                "is_pivot_good: No current price available for %s", self.ticker
            )
            return False, None, None, None

        flag = (self.m_footPrint[-1][2] <= algoParas.PIVOT_PRICE_PERC) and (
            current > self.m_recordVCP[-1][3]
        )
    ...
    def is_demand_dry(self):
        ...
        priceDataStruct = self.priceData[self.ticker]["prices"]
        footprintVolume = self._extract_volume_for_period(
            priceDataStruct, startDate_dt.date(), endDate_dt.date()
        )
        ...
        slopeRecentPrice, _ = self._calculate_volume_trend(
            [item["close"] for item in recentData]
        )
    ...
    def _extract_volume_for_period(self, priceDataStruct, start_date, end_date):
        ...
        selected_data = self.get_price_from_buffer_start_end(
            priceDataStruct, start_date, end_date
        )
    ...
    def _calculate_historical_average_volume(self, priceDataStruct, days):
        ...
        historical_data = self.get_price_from_buffer_start_end(
            priceDataStruct, start_date, end_date
        )
...
class batch_process:
    ...
    def __init__(self, tickers, sectors, market=None):
        ...
        logger.info(
            "Initialized batch_process for %d tickers; results -> %s, csv -> %s",
            len(self.tickers),
            self.result_file,
    ...
    def batch_strategy(self):
        ...
        for i in range(total):
            try:
                ...
                pass
        logger.info(
            "batch_strategy finished; candidates=%d, elapsed=%.2fs",
            len(superStock),
    ...
    def batch_pipeline_full(self):
        ...
        if PREFETCH_ENABLED and total > 1:
            try:
                ...
                logger.info(
                    "Prefetching last %d days of historical price data concurrently for %d tickers",
                    days,
                    total,
                )
                try:
                    ...
                    for ticker in self.tickers:
                        try:
                            ...
                            logger.debug(
                                "Failed to process prefetch for %s",
                                ticker,
                                exc_info=True,
                            )
        ...
        for idx in range(total):
            try:
                ...
                try:
                    ...
                    if price_map:
                        x = cookFinancials(
                            ticker,
                            priceData=price_map,
                            fetch_days=HISTORICAL_DAYS_DEFAULT,
                        )
                    else:
                        x = cookFinancials(ticker)
                    ...
                    try:
                        ...
                        counter, record = x.find_volatility_contraction_pattern(
                            date_from
                        )
                        footprint = x.get_footPrint()
                        isGoodPivot, currentPrice, supportPrice, pressurePrice = (
                            x.is_pivot_good()
                        )

                        if (
                            currentPrice is None
                            or supportPrice is None
                            or pressurePrice is None
                        ):
                        ...
                        logger.exception(
                            "Error in analysis for %s, using defaults", ticker
                        )
                        supportPrice = currentPrice if currentPrice else 0
                        pressurePrice = currentPrice if currentPrice else 0
                        isGoodPivot = False
                    ...
                    logger.info(
                        "get_price for %s finished in %.2fs", ticker, time.time() - t1
                    )
                    tmpLen = len(sp)
                    date = []
                    price = []
                    ...
                    if (
                        len(date) - 1 not in xticks
                    ):  # Check if the last date is already included
                        xticks.append(
                            len(date) - 1
                        )  # Add the last date index to x-ticks
                    ...
                    logger.info(
                        "Highest in 5 days for %s: %s",
                        ticker,
                        x.get_highest_in5days(date_from),
                    )
                    ...
                    ticker_data = {
                        ticker: {
                            ...
                            "swing_trade_entry": str(isSwingEntry),
                            "ema_8": str(swingDetails.get("ema_8", "N/A")),
                            "sma_200": str(swingDetails.get("sma_200", "N/A")),
                        }
                    }
                    ...
                    if counter > 0:
                        ...
                        if volume_ls:
                            for ind, item in enumerate(date):
                                if item == startDate:
                                    logger.info(
                                        "start index for demand dry for %s: %d",
                                        ticker,
                                        ind,
                                    )
                                    break
                            ...
                            if volume_re:
                                for ind, item in enumerate(date):
                                    if item == recentStart:
                                        logger.info(
                                            "recent start index for %s: %d", ticker, ind
                                        )
                                        break
                    ...
                    img_folder = self.image_folders.get(
                        market, self.image_folders["US"]
                    )
                    ...
                    if counter > 0 or (isGoodPivot and not (isDeepCor) and isDemandDry):
                        fig.savefig(
                            figName, format="jpeg", dpi=100, bbox_inches="tight"
                        )
                        logger.info("Saved figure %s", figName)
                        ticker_data[ticker]["fig"] = figName
                    else:
                        figName = (
                            ""  # No chart saved if no VCP and doesn't meet criteria
                        )
                    ...
                    logger.info(
                        "combined_best_strategy for %s: %s (finished in %.2fs)",
                        ticker,
                        flag,
                        time.time() - t0,
                    )
                    ...
                    append_to_csv(
                        csv_file,
                        ticker,
                        currentPrice if currentPrice else 0,
                        supportPrice,
                        pressurePrice,
                        isGoodPivot,
                        isDeepCor,
                    ...
                    heartbeat.set()
                    logger.info(
                        "Processing complete for %s; elapsed=%.2fs",
                        ticker,
                        time.time() - start_t,
                    )
            except Exception:
                ...
                pass
        logger.info(
            "batch_pipeline_full finished; candidates=%d, elapsed=%.2fs",
            len(superStock),
            time.time() - start_time,
        )
    ...
    try:
        ...
        if support_val != 0:
            price_to_support = round(
                ((float(current_price) - support_val) / support_val) * 100, 2
            )
        else:
            price_to_support = 0
